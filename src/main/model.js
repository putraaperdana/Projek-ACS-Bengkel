...
export const completeRepair = async (event, payload) => {
   ...
   try {
      const [rows] = await pool.query('CALL SP_SelesaikanPerbaikan(?,?,?)', [id_log, odometer_baru, compJson]);
      return { success: true };
   } catch (err) {
...
export const addSukuCadang = async (event, payload) => {
   ...
   try {
      const [res] = await pool.query('INSERT INTO Suku_Cadang (nama,kategori,kuantitas_fisik,batas_minimum) VALUES (?,?,?,?)', [nama, kategori, kuantitas_fisik || 0, batas_minimum || 0]);
...
export const addKendaraan = async (event, payload) => {
   ...
   try {
      const [res] = await pool.query('INSERT INTO Kendaraan_Operasional (nomor_polisi,tahun,odometer,status) VALUES (?,?,?,?)', [nomor_polisi, tahun || null, odometer || 0, status || 'Aktif']);
      return { success: true, insertId: res.insertId };
   } catch (err) {
      throw new Error(err.sqlMessage || err.message || 'DB insert error');
   }
...
export const addLogPerbaikan = async (event, payload) => {
   ...
   try {
      const [res] = await pool.query('INSERT INTO Log_Perbaikan (nomor_polisi,id_mekanik,tanggal,status) VALUES (?,?,NOW(),?)', [nomor_polisi, id_mekanik || null, 'Diperbaiki']);
      // Also set kendaraan status to Diperbaiki
      await pool.query('UPDATE Kendaraan_Operasional SET status = ? WHERE nomor_polisi = ?', ['Diperbaiki', nomor_polisi]);
      return { success: true, insertId: res.insertId };
   } catch (err) {
      throw new Error(err.sqlMessage || err.message || 'DB insert error');
   }
...
export const deleteKendaraan = async (event, nomor_polisi) => {
   try {
      const [res] = await pool.query('DELETE FROM Kendaraan_Operasional WHERE nomor_polisi = ?', [nomor_polisi]);
      if (res.affectedRows === 0) throw new Error('Kendaraan tidak ditemukan');
      return { success: true };
   } catch (err) {
      // Foreign key restriction or other sql errors
      if (err && err.code === 'ER_ROW_IS_REFERENCED_2') {
         throw new Error('Kendaraan memiliki referensi log perbaikan; hapus atau batalkan log terlebih dahulu');
      }
...
export const login = async (event, username, password) => {
   try {
      const [resultSets] = await pool.query('CALL SP_LoginMekanik(?,?)', [username, password]);
      const rows = Array.isArray(resultSets) && Array.isArray(resultSets[0]) ? resultSets[0] : resultSets;
