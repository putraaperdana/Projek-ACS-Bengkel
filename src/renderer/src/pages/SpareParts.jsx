...
export default function SpareParts() {
  ...
  const submit = async () => {
    if (!form.nama || !form.kategori) return setToast({ open: true, message: 'Isi nama dan kategori', severity: 'warning' });
    setLoading(true);
    try {
  ...
  return (
    <Container sx={{ py: 3 }}>
      ...
      <Box sx={{ my: 2, display: 'flex', gap: 1 }}>
        <TextField label="Nama" value={form.nama} onChange={e => setForm({ ...form, nama: e.target.value })} sx={{ flex: 1 }} />
        <TextField label="Kategori" value={form.kategori} onChange={e => setForm({ ...form, kategori: e.target.value })} sx={{ width: 200 }} select>
          {kategoriList.map(k => <MenuItem key={k} value={k}>{k}</MenuItem>)}
          <MenuItem value={form.kategori}>{form.kategori}</MenuItem>
        </TextField>
        <TextField label="Stok" type="number" value={form.kuantitas_fisik} onChange={e => setForm({ ...form, kuantitas_fisik: parseInt(e.target.value||0,10) })} sx={{ width: 120 }} />
        <TextField label="Batas Min" type="number" value={form.batas_minimum} onChange={e => setForm({ ...form, batas_minimum: parseInt(e.target.value||0,10) })} sx={{ width: 120 }} />
        <Button variant="contained" onClick={submit} disabled={loading}>{loading ? <CircularProgress size={18} /> : 'Tambah'}</Button>
      </Box>

      <Box sx={{ my: 2, display: 'flex', gap: 1 }}>
        <TextField select label="Filter Kategori" value={selectedKategori} onChange={e => { setSelectedKategori(e.target.value); loadSuku(e.target.value); }} sx={{ width: 300 }}>
