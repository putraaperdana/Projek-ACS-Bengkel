...
export default function WorkOrders(){
  ...
  const create = async ()=>{
    ...
    const res = await window.electron.ipcRenderer.invoke('workorders:create', Number(customerId), null, JSON.stringify(selectedParts))
    ...
    <div style={{padding:20}}>
      ...
      <div>
        ...
        <select value={customerId} onChange={e=>setCustomerId(e.target.value)}>
          ...
          {customers.map(c=> <option key={c.id} value={c.id}>{c.full_name} ({c.vehicle_plate || ''})</option>)}
        </select>
      </div>
      <div style={{marginTop:10}}>
        <h4>Parts</h4>
        <ul>
          {parts.map(p=> <li key={p.id}>{p.name} ({p.stock_qty}) <button onClick={()=>addPart(p.id)}>Add</button></li>)}
        </ul>
      </div>
      <div>
        <h4>Selected</h4>
        <ul>
          {selectedParts.map((s,i)=> <li key={i}>Part {s.part_id} - Qty: {s.quantity}</li>)}
        </ul>
      </div>
